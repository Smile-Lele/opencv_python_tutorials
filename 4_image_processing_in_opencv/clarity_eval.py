...
def set_params(cap, fps=18, exposure=-4):
...
def get_params(cap):
    ...
    exposure = cap.get(cv.CAP_PROP_EXPOSURE) # exposure: from -13 to -1
...
def grab(cap, retFrames=0, record=False, visualized=True):
    ...
    while cap.isOpened():
        ...
        if font_neibour < 150:
            font_color = 255
        cv.putText(img, "Clarity:{}".format(round(clarity_sobel,2)), (15, 50), cv.FONT_HERSHEY_SIMPLEX, 1.0, font_color, 2)
